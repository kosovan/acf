/* A U T O C O R R E L A T O R Version 0.9.9, 22.07.2010 by PK */

/* This code almost works fine except for the single fixme */ 
...
FIXME
* each blocking operation introduces a step in the result
* is it an artifact of the algorithm which vanishes with lots of data or is it a bug ?
...
int dump_res(FILE *f, double *result, int n_res, int nc, int gs, double dt, int n_vals, int n_part) {
	...
	for(i=0;i<n_fac;i++) fac[i]=0.0;

	fprintf(f,"#time\t product(acf)\n");
	for(i=0; i<n_res; i++){
		// correct for the systematic error due to coarse-graining
		// FIXME the correction only works properly when maximum blocking level < 2*gs
		// In other cases we have to care about the time 
		act_n_vals=n_part*(n_vals-d);
		if(i==next_i && fac_index<n_fac) {
			fac[fac_index]=0.5*result[i]/act_n_vals;
			fac_index++;
			next_i*=nc;
		}
		printf("adding %lf=0.5*%lf to the result at t=%lf\n",2*fac[c_index],fac[c_index],t);
		fprintf(f, "%e\t %e\t %d \t %d\n", t, result[i]/act_n_vals+fac[c_index], act_n_vals, n_vals_hist[i]);
		t+=dt;
